...
export const StudentNotices: React.FC = () => {
  ...
  const handleFetchNotices = async () => {
    ...
    try {
      ...
      toast.success(res.message || 'Notices refreshed');
    } catch (err) {
  ...
  const handleFetchDetail = async () => {
    ...
    setLoading(true);
    try {
      const res = await studentService.getNoticeById(noticeId);
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div>
        ...
        <p className="text-muted-foreground text-xs">
          Official circulars, holiday announcements, and event notifications.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Search circulars by keyword</CardDescription>
          </CardHeader>
            ...
            {notices && (
              <div className="max-h-48 space-y-2 overflow-y-auto pt-2">
                {filtered && filtered.length > 0 ? (
                  ...
                  <p className="text-muted-foreground text-xs">No circulars match filter.</p>
                )}
              </div>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Notice Detail Reader</CardTitle>
            <CardDescription>Open a specific notice by Notice ID</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
