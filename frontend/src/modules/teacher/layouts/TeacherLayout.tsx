...
const navigation = [
  ...
  { name: 'Attendance & Registry', href: 'attendance', icon: ClipboardCheck },
  { name: 'Exams List', href: 'exams', icon: Calendar },
  { name: 'Results Entry', href: 'results', icon: Award },
  { name: 'Notice Board', href: 'notices', icon: Bell },
  { name: 'Salary History', href: 'salary', icon: DollarSign },
];
    ...
    <div className="bg-muted/20 text-foreground min-h-screen">
      ...
      <header className="bg-background sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b px-4 shadow-sm md:px-8">
        <div className="flex items-center gap-4">
          ...
          <span className="hidden items-center rounded-full bg-violet-100 px-2.5 py-0.5 text-xs font-semibold text-violet-700 md:inline-flex dark:bg-violet-900/30 dark:text-violet-400">
            Teacher Portal
          </span>
        </div>
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-10 w-10 rounded-full">
                <Avatar className="h-10 w-10">
                  <AvatarFallback className="bg-violet-100 font-semibold text-violet-700 dark:bg-violet-900/30 dark:text-violet-400">
                    {user ? getInitials(user.username) : 'TC'}
                  </AvatarFallback>
                </Avatar>
              </Button>
            </DropdownMenuTrigger>
        ...
        <aside className="bg-background sticky top-16 hidden min-h-[calc(100vh-4rem)] w-64 space-y-2 border-r p-4 md:block">
          ...
          <nav className="space-y-1">
            {navigation.map((item) => {
              ...
              const isActive = location.pathname.includes(`/teacher/${item.href}`);
              return (
                <Link
                  key={item.name}
                  to={item.href}
