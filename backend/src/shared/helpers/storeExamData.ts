...
export const storeExamData = async (data: z.infer<typeof CreateExamSchema>) => {
    await prisma.$transaction(async (tx) => {
        ...
        for (const examData of exams) {
            ...
            const subjectMap = new Map<string, { id: string; teacherId: string | null; subjectCode: string }>(
                subjectRecords.map(s => [s.subject.subjectCode, { id: s.subject.id, teacherId: s.teacherId, subjectCode: s.subject.subjectCode }])
            );
